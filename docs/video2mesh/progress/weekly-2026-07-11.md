...
category: 进度目录
visibility: public
summary: 覆盖 2026-07-04 至 2026-07-11：整理高质量 3DGS/mesh 路线对比，完成 SimFoundry 风格资产六件套，部署 GenRecon/PGSR，开展 Holi-Spatial 方法调研，归档 SceneVerse++ / PQ3D 结果，并更新 Web demo 与点云补全调研。
tags:
  - Weekly
  - 3DGS
  - Mesh
...
| [PGSR](#/doc/video2mesh-visual-3dgs-pgsr) | 当前保留的 fresh 30k run：PSNR 33.1155；871,317 Gaussians；TSDF post mesh 694,773 vertices / 1,351,454 faces | 主体空洞改善、TSDF 表面连续；边缘仍需 floater/elongation 过滤，不能直接替代 collider |
...
### PGSR：fresh 30k 的主体质量可用，边缘仍需清理

![PGSR bedroom_4 fresh TSDF mesh](../experiments/assets/holi-spatial-bedroom4-rerun-20260714-tsdf-mesh.png "当前保留的 PGSR 30k TSDF mesh：床、墙、窗和地面连续，主体几何稳定")

![PGSR bedroom_4 fresh Gaussian PLY](../experiments/assets/holi-spatial-bedroom4-rerun-20260714-pgsr-30k-gaussians.png "当前保留的 PGSR 30k Gaussian：主体空洞改善，边缘仍有少量拉丝和漂浮点")

[PGSR](#/doc/video2mesh-visual-3dgs-pgsr) 的当前唯一保留结果是 fresh 30k run。它跑通 CUDA extension、训练、render、depth/normal 输出和 TSDF mesh export，并把 vanilla 3DGS 推向 surface-aware reconstruction，可作为 Holi-Spatial-style 2D-to-3D lifting、bbox 和 QA 的几何后端候选。

该 run 的 train-view PSNR 为 `33.1155 dB`，床和房间主体空洞已基本修复，TSDF mesh 的床、墙、窗和地面连续。原始 Gaussian 在场景外缘仍有拉丝、漂浮点与少量放射状伪影；因此它可作为 geometry / visual mesh 候选，尚不能直接替代 GraphDECO visual layer 或 collider。
...
[Holi-Spatial](#/doc/video2mesh-semantic-scene-graph-holi-spatial) 的价值是把 DA3、PGSR、VLM、SAM3、2D-to-3D lifting、bbox 和空间 QA 组织成可审计的数据生成链路。此前的 smoke / proxy 输出已删除；当前唯一保留的实测结论位于 [2026-07-14 fresh 全链路实验](#/doc/video2mesh-experiments-holi-spatial-bedroom4-fresh-run-20260714)。
