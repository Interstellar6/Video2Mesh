...
P0 的目标是展示和交互闭环，不是最佳画质：COLMAP dense + Delaunay mesh 做场景 collider，GraphDECO 3DGS 做 visual layer，语义与物理属性通过 sidecar 管理。默认语义路线使用 2D mask probability backprojection 写 binary semantic core，并从中生成 semantic mesh；backprojection 显式绑定当前 active `scene_3dgs_ply`，并把 source/output 的 `means/opacities/scales/quats` 指纹写入 manifest。SuperSplat 检查时打开 scene 3DGS 加轻量 semantic overlay，不直接加载 full semantic core。默认 overlay 只保留高置信语义 Gaussian，最多 180,000 个，且不会生成第二份完整 semantic SuperSplat 复制件；若原始 Gaussian 数值不安全，overlay 使用 display-only viewer-safe shape arrays。所有 mesh 同时输出单面 collider/source 和双面 display companion，避免 viewer 的 backface culling 改变质量判断。AnySplat 等独立 Gaussian 路线必须使用自己的预测相机和预处理画幅，外参从 camera-to-world 反转后才可进入该回投器，并且不覆盖主 manifest。
