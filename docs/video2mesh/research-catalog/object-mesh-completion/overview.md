...
category: 调研目录
summary: 梳理 Hunyuan3D、Meshy、TRELLIS、InstantMesh、NOVA3R、Stream3D、image-blaster 等物体级生成和补全方案。
tags:
  - Research Catalog
  - Object Mesh
  - Hunyuan3D
...
| Stream3D | 冻结 SAM3D / TRELLIS.2 的训练免流式多视角生成包装层 | posed object views、逐帧 mask；SAM3D 分支还需 DA3 depth / intrinsics / poses，到逐 chunk Gaussian PLY 与 visual mesh GLB | 适合作为 object track 后的长视频证据聚合与 amodal 视觉补全后端 | 不是场景级持续建图；room alignment、collider、semantic / physics sidecar 仍需 Video2Mesh 处理 |
...
object masks / selected frames
  ...
  -> Hunyuan3D / Meshy / TRELLIS / NOVA3R / Stream3D / InstantMesh / Restore3D
