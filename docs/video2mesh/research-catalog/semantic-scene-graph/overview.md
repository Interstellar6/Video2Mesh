...
| [SAM3-I](sam3-i.md) | 在冻结 SAM3 上加入级联指令适配器，直接理解属性、关系、动作、功能和隐式推理指令 | 可替代 `Qwen 拆短词 -> SAM3 -> 多轮过滤`，生成 instance-specific 2D mask evidence | 主结果依赖作者数据；one-to-many 与视频一致性仍弱；代码和数据许可证不完整 |
