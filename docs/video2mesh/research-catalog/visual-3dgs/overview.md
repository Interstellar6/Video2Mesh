...
| PGSR | 在 3DGS 中加入几何/平面一致性约束，尝试让 Gaussian 更接近可抽 mesh 的表面 | P1/P2 geometry-aware 3DGS 对照；可验证 rendered depth -> TSDF fusion 路线 | 当前 bedroom_4 120 iter smoke 质量还不足以替代 visual layer 或 collider |
...
`bedroom_4` 的 PGSR smoke 进一步强化了这个边界：PGSR 部署、120 iter 训练和 TSDF mesh export 已跑通，但 `point_cloud.ply` 仍有明显雾状 splats 和漂浮块，`tsdf_fusion_raw/post.ply` 仍有薄片、粘连和破洞。它目前适合作为 geometry-aware 3DGS 研究候选，不适合直接替换 Video2Mesh 当前 visual layer 或 collider 主路线。
